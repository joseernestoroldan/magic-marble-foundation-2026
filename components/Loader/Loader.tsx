...
const Loader = ({ variant = "full" }: { variant?: "full" | "inline" }) => {
  if (variant === "inline") {
    return (
      <span style={{ display: "inline-flex", animation: "inlineSpin 1s linear infinite" }}>
        <svg
        ...
        <style>{`@keyframes inlineSpin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}`}</style>
      </span>
    );
  }
